...
r = requests.get('https://totstories.typlog.io')
...
for div in soup.find_all(attrs={"class": "item"}):
    ...
    r_deep = requests.get('https://totstories.typlog.io/'+href)
    soup_deep = BeautifulSoup(r_deep.text, 'html.parser')
    text = soup_deep.find(attrs={"class": "e-content"})

#     inner

    new = href+'/{}.html'.format(filename)
    print(new)
